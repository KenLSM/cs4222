...
#include <stdint.h>
/*---------------------------------------------------------------------------*/
#define CC26XX_DEMO_LOOP_INTERVAL       (CLOCK_SECOND * 0.001)
...
static bool is_weird(int reading);
static void set_weird();
...
static void get_mpu_reading(){
  ...
  if (is_weird(accel_data[0].x) || is_weird(accel_data[0].y) || is_weird(accel_data[0].z)){
    skip = true;
...
static bool is_weird(int reading) {
  return reading == -2147483648;
}

static void set_weird() {
  accel_data[0].x = -2147483648;
  accel_data[0].y = -2147483648;
  accel_data[0].z = -2147483648;
}
        ...
        if(write_to_ext_flash){
          ...
          if(EXT_FLASH_BASE_ADDR_SENSOR_DATA + address_index_writing + sizeof(sensor_data_int) < EXT_FLASH_MEMORY_END_ADDRESS){
            if (!skip) {
              ...
              rv = ext_flash_write(
                EXT_FLASH_BASE_ADDR_SENSOR_DATA + address_index_writing, 
                sizeof(sensor_data_int), 
                (uint8_t *)&sensor_data_int);
              if(!rv) {
